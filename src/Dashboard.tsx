...
export default function Dashboard(){
  ...
  return <div className={`dashboard ${loading?'is-refreshing':''}`}>
    ...
    <section className="summary-grid">
      <SummaryCard label="Encaissé" value={money(data.summary.paid)} detail={`${collectedRate} % du facturé`} progress={collectedRate}/>
      <SummaryCard label="Reste à encaisser" value={money(data.summary.receivables)} detail="sur les factures de la période"/>
      <SummaryCard label="Factures émises" value={String(data.summary.invoices)} detail={`panier moyen ${money(data.summary.averageBasket)}`}/>
      <SummaryCard label="Stock en alerte" value={String(data.stock.low+data.stock.out)} detail={`${data.stock.out} en rupture, ${data.stock.low} au plus bas`} warn={data.stock.out>0}/>
      <SummaryCard label="Valeur du stock" value={money(data.stock.value)} detail={`${data.stock.units} unités · ${data.stock.variants} références`}/>
    </section>

    <ChartCard className="trend-card" title="Facturé et encaissé, période par période" subtitle="L’écart entre les deux courbes représente ce qui reste à récupérer.">
      <div className="chart-large"><ResponsiveContainer width="100%" height="100%"><AreaChart data={data.trend} margin={{top:16,right:12,left:0,bottom:0}}>
        <defs><linearGradient id="billedArea" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor="#292d35" stopOpacity=".18"/><stop offset="100%" stopColor="#292d35" stopOpacity=".01"/></linearGradient></defs>
        <CartesianGrid stroke="#ececef" vertical={false}/><XAxis dataKey="date" tickFormatter={v=>dateLabel(String(v),period)} axisLine={false} tickLine={false} minTickGap={26}/><YAxis tickFormatter={v=>compact(Number(v))} axisLine={false} tickLine={false} width={54}/>
        <Tooltip content={<MoneyTooltip/>}/><Area type="monotone" dataKey="billed" name="Facturé" stroke="#252930" strokeWidth={2.4} fill="url(#billedArea)"/><Area type="monotone" dataKey="paid" name="Encaissé" stroke="#8c929c" strokeWidth={2} fill="transparent"/>
      </AreaChart></ResponsiveContainer></div><ChartLegend items={[['#252930','Facturé'],['#8c929c','Encaissé']]}/>
    </ChartCard>

    <section className="analytics-grid">
      <ChartCard title="Créances par ancienneté" subtitle="Tout l’impayé en cours, quelle que soit la période."><SmallBars data={data.ageing.map(x=>({name:x.label,value:x.value}))}/></ChartCard>
      <ChartCard title="Statut d’encaissement" subtitle="Répartition des factures de la période."><PaymentStatus statuses={data.paymentStatus}/></ChartCard>
      <ChartCard title="Meilleurs produits" subtitle="Chiffre d’affaires sur la période."><Ranking data={data.topProducts}/></ChartCard>
      <ChartCard title="Meilleurs clients" subtitle="Chiffre d’affaires facturé sur la période."><Ranking data={data.topCustomers}/></ChartCard>
      <ChartCard title="Ventes par catégorie" subtitle="Part de chaque famille de produits."><CategoryChart data={data.categories}/></ChartCard>
      <ChartCard className="traffic-card" title="Affluence par jour et par heure" subtitle="Plus la case est foncée, plus l’heure est chargée."><Heatmap data={data.traffic}/></ChartCard>
      <ChartCard className="stock-card" title="Articles à réapprovisionner" subtitle={`${data.stock.out} rupture${data.stock.out!==1?'s':''} et ${data.stock.low} stock${data.stock.low!==1?'s':''} faible${data.stock.low!==1?'s':''}.`}><StockAlerts items={data.stock.alerts}/></ChartCard>
      <ChartCard className="customer-card" title="Portefeuille client" subtitle="Vue rapide de la base clients."><div className="customer-summary"><span><Users/></span><div><strong>{data.customers.total}</strong><small>clients enregistrés</small></div><div><strong>{data.orders}</strong><small>commandes web</small></div></div></ChartCard>
    </section>
  </div>;
}

function SummaryCard({label,value,detail,progress,warn}:{label:string;value:string;detail:string;progress?:number;warn?:boolean}){return <article className="summary-card"><span>{label}</span><strong>{value}</strong><small className={warn?'warning-text':''}>{detail}</small>{progress!==undefined&&<div className="summary-progress"><i style={{width:`${Math.min(100,progress)}%`}}/></div>}</article>}
function ChartCard({title,subtitle,children,className='' }:{title:string;subtitle:string;children:React.ReactNode;className?:string}){return <article className={`chart-card ${className}`}><header><h2>{title}</h2><p>{subtitle}</p></header>{children}<button className="show-values"><ChevronDown/>Voir les valeurs</button></article>}
...
function SmallBars({data}:{data:{name:string;value:number}[]}){const source=data.length?data:[{name:'1–30 j',value:0},{name:'31–60 j',value:0},{name:'61–90 j',value:0},{name:'+90 j',value:0}];return <div className="chart-small"><ResponsiveContainer width="100%" height="100%"><BarChart data={source}><CartesianGrid stroke="#eeeeef" vertical={false}/><XAxis dataKey="name" axisLine={false} tickLine={false}/><YAxis tickFormatter={v=>compact(Number(v))} axisLine={false} tickLine={false} width={45}/><Tooltip formatter={tooltipMoney}/><Bar dataKey="value" radius={[5,5,0,0]}>{source.map((_,i)=><Cell key={i} fill={['#c9cdd3','#999fa8','#626872','#292d35'][i%4]}/>)}</Bar></BarChart></ResponsiveContainer></div>}
function PaymentStatus({statuses}:{statuses:Status[]}){const config={paid:{label:'Payée',color:'#2f343b'},partial:{label:'Partiellement payée',color:'#999fa8'},pending:{label:'En attente',color:'#d8dade'}};const total=statuses.reduce((s,x)=>s+x.count,0);return <div className="payment-status"><div className="status-bar">{statuses.map(x=><i key={x.status} style={{width:`${total?x.count/total*100:0}%`,background:config[x.status].color}}/>)}</div>{(['paid','partial','pending'] as const).map(status=>{const row=statuses.find(x=>x.status===status);return <div className="status-row" key={status}><i style={{background:config[status].color}}/><span>{config[status].label} ({row?.count??0})</span><strong>{money(row?.value??0)}</strong></div>})}</div>}
...
function StockAlerts({items}:{items:Alert[]}){if(!items.length)return <div className="all-good"><Box/><strong>Stock sous contrôle</strong><span>Aucun article sous son seuil.</span></div>;return <div className="stock-alerts">{items.map(item=><div key={item.id}><span className={item.stock<=0?'out':''}><AlertTriangle/></span><div><strong>{item.product}</strong><small>{item.sku}</small></div><div className="stock-meter"><i style={{width:`${Math.max(4,Math.min(100,item.alertAt?item.stock/item.alertAt*100:0))}%`}}/></div><b>{item.stock}</b></div>)}</div>}
