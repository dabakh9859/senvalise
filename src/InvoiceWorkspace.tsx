...
// rassure.
const legalLine=(info:{ninea?:string;tradeRegister?:string;legalNote?:string})=>[
  info.ninea?`NINEA : ${info.ninea}`:'',info.tradeRegister?`RC : ${info.tradeRegister}`:'',info.legalNote??''
].filter(Boolean).join('  ·  ');
...
export default function InvoiceWorkspace(props:Props){
  ...
  // Ce que le vendeur peut modifier sur une pièce, et rien de plus.
  //
  // La fiche entière était derrière le drapeau « gérant » : le vendeur voyait
  // une facture en lecture seule, sans même pouvoir y enregistrer un
  // règlement — alors que le serveur l'y autorise, et que relancer les
  // créances fait partie de son travail. Ce qui reste au gérant, ce sont les
  // gestes que le serveur lui réserve : toucher aux lignes, annuler un
  // règlement encaissé, signer, supprimer la pièce.
  const vendorSections=new Set(['client','payment','general']);
  const mayEdit=(section:string)=>props.isAdmin||(props.canSettle&&vendorSections.has(section));
  const openEdit=(section:string)=>{if(!mayEdit(section))return;if(!brandingEditable&&(section==='branding'||section==='content'))return;setError('');setEditing(section)};
...
function EditDrawer({title,editing,kind,form,setForm,customers,variants,usedVariantIds,payments,remaining,editLine,canDeleteLine,onDeleteLine,storedSignature,signed,isAdmin,saving,error,onClose,onSubmit,onCancelPayment,onCancelAll,onToggleSignature,onReplaceSignature}:{title:string;editing:string;kind:Kind;form:Record<string,string>;setForm:(form:Record<string,string>)=>void;customers:Entity[];variants:Variant[];usedVariantIds:number[];payments:Payment[];remaining:number;editLine?:Line;canDeleteLine:boolean;onDeleteLine:(line:Line)=>void;storedSignature:string;signed:boolean;isAdmin:boolean;saving:boolean;error:string;onClose:()=>void;onSubmit:(event:FormEvent)=>void;onCancelPayment:(payment:Payment)=>void;onCancelAll:()=>void;onToggleSignature:(value:boolean)=>Promise<void>;onReplaceSignature:(file:File)=>Promise<void>}){
  ...
  return <aside className="invoice-edit-drawer"><header><div><small>MODIFICATION</small><h2>{title}</h2><p>La pièce reste visible pendant la modification.</p></div><button className="icon" onClick={onClose}><X/></button></header>{error&&<div className="error">{error}</div>}<form onSubmit={onSubmit}>
    ...
    {editing==='payment'&&<PaymentPanel payments={payments} remaining={remaining} form={form} field={field} saving={saving} isAdmin={isAdmin} onCancel={onCancelPayment} onCancelAll={onCancelAll}/>} 
...
function PaymentPanel({payments,remaining,form,field,saving,isAdmin,onCancel,onCancelAll}:{payments:Payment[];remaining:number;form:Record<string,string>;field:(name:string,value:string)=>void;saving:boolean;isAdmin:boolean;onCancel:(payment:Payment)=>void;onCancelAll:()=>void}){
  const active=payments.filter(row=>row.status!=='cancelled');return <><div className="payment-balance"><span>Reste à payer</span><strong>{money(remaining)}</strong></div>{remaining>0&&<div className="payment-add"><h3><Plus/>Ajouter un règlement</h3><label>Mode de paiement<select value={form.paymentMethod} onChange={event=>field('paymentMethod',event.target.value)}><option value="cash">Espèces</option><option value="wave">Wave</option><option value="orange_money">Orange Money</option><option value="card">Carte bancaire</option><option value="bank_transfer">Virement</option></select></label><NumberField label="Montant" value={form.paymentAmount} onChange={value=>field('paymentAmount',value)} min={1} max={remaining}/></div>}<div className="payment-history-head"><h3>Historique des règlements</h3>{isAdmin&&active.length>1&&<button type="button" disabled={saving} onClick={onCancelAll}><Ban/>Tout annuler</button>}</div><div className="payment-history">{payments.length===0?<p>Aucun règlement enregistré.</p>:payments.map(row=><article key={row.id} className={row.status==='cancelled'?'cancelled':''}><div><strong>{money(row.amount)}</strong><span>{methodLabel(row.method)} · {row.reference}</span><small>{longDate(row.createdAt)}{row.status==='cancelled'?' · Annulé':''}</small></div>{isAdmin&&row.status!=='cancelled'&&<button type="button" onClick={()=>onCancel(row)}><RotateCcw/>Annuler</button>}</article>)}</div></>;
}
