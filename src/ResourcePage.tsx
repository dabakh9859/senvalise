...
export default function ResourcePage({title,resource,user}:Props){
  ...
  const uploadSignature=async(kind:'client'|'company',file:File)=>{if(!dialog?.row)return;const target=dialog.resource??resource;const body=new FormData();body.append('image',file);await apiForm(`${endpoint(target,dialog.row.id)}/signatures/${kind}`,body);await refreshDialog(target)};
